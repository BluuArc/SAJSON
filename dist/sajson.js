...
function call_dispatchWindowEvent(eventName,eventKey) { const eventNameAsString = UTF8ToString(eventName); const eventKeyAsString = UTF8ToString(eventKey); console.log("dispatching event", { eventNameAsString, eventKeyAsString }); const event = new CustomEvent(eventNameAsString, { detail: { key: eventKeyAsString } }); if (typeof window !== "undefined") { window.dispatchEvent(event); } else { self.dispatchEvent(event); } }
...
var wasmImports = {
  ...
  abort: _abort,
  /** @export */
  call_dispatchWindowEvent: call_dispatchWindowEvent,
  /** @export */
